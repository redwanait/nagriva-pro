...
window.NAGRIVA_Dashboard = (function () {
  ...
  function renderStats(stats) {
    ...
    items.forEach(function (item, i) {
      ...
      card.className = 'dash-stat-card dash-fade-in';
      card.innerHTML =
        ...
        '<span class="dash-stat-change">' + item.change + '</span>';
  ...
  function renderOrders(orders) {
    ...
    if (!orders || orders.length === 0) {
      container.innerHTML = emptyOrders();
      return;
    }
    container.innerHTML = '';
  ...
  function renderActivity(activities) {
    ...
    if (!activities || activities.length === 0) {
      container.innerHTML = emptyActivity();
      return;
    }
    container.innerHTML = '';
    activities.slice(0, 5).forEach(function (a) {
      ...
      item.innerHTML =
        ...
        '<span class="dash-activity-time">' + formatTimeAgo(a.created_at) + '</span>';
      container.appendChild(item);
    });
  }
  ...
  function renderMessages(messages) {
    ...
    if (!messages || messages.length === 0) {
      container.innerHTML = emptyMessages();
      return;
    }
    container.innerHTML = '';
    var grouped = {};
    ...
    latest.slice(0, 4).forEach(function (m) {
      ...
      item.className = 'dash-message-item';
      item.href = 'order-tracking.html?id=' + (m.order_id || '');
      item.innerHTML =
        ...
        '</div>' +
        '<span class="dash-message-time">' + formatTimeAgo(m.created_at) + '</span>';
      container.appendChild(item);
    });
  }
  ...
  function renderNotifications(notifications) {
    ...
    if (!notifications || notifications.length === 0) {
      container.innerHTML = emptyNotifications();
      return;
    }
    container.innerHTML = '';
    notifications.slice(0, 4).forEach(function (n) {
      var item = document.createElement('div');
      item.className = 'dash-notif-item' + (n.is_read ? '' : ' unread');
      ...
      item.innerHTML =
        ...
        '</div>' +
        '<span class="dash-notif-time">' + formatTimeAgo(n.created_at) + '</span>';
      container.appendChild(item);
    });
  }
  ...
  function renderProjects(projects) {
    ...
    if (!projects || projects.length === 0) {
      container.innerHTML = emptyProjects();
      return;
    }
    container.innerHTML = '';
    projects.slice(0, 6).forEach(function (p) {
      ...
      card.innerHTML =
        ...
        '<span class="dash-project-status ' + getStatusBadgeClass(p.status) + '">' + getStatusLabel(p.status) + '</span>';
  ...
  function showSkeletons() {
    ...
    if (statsGrid) statsGrid.innerHTML = skeletonStats();
    var ordersEl = document.getElementById('dashOrdersList');
    if (ordersEl) ordersEl.innerHTML = skeletonList(3);
    var activityEl = document.getElementById('dashActivity');
    if (activityEl) activityEl.innerHTML = skeletonList(3);
    var messagesEl = document.getElementById('dashMessages');
    if (messagesEl) messagesEl.innerHTML = skeletonList(3);
    var notifsEl = document.getElementById('dashNotifications');
    if (notifsEl) notifsEl.innerHTML = skeletonList(3);
    var projectsEl = document.getElementById('dashProjects');
    if (projectsEl) projectsEl.innerHTML = skeletonProjects(3);
  }
  ...
  async function init() {
    ...
    try {
      ...
      containers.forEach(function (id) {
        var el = document.getElementById(id);
        if (el && (el.querySelector('.dash-skeleton') || el.querySelector('.dash-stat-skel'))) {
